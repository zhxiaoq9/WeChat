异步读写指针的比较一直是异步FIFO中需要重点关注的内容。在“[FIFO设计实例1](https://github.com/zhxiaoq9/WeChat/blob/master/ClassicalCircuitDesign_FIFO/FIFOExample1.md)”中采取的方案是将另一个时钟域的读写指针同步过来后进行比较。这是一种同步的比较方案，即读写指针的比较是在时钟有效沿比较的。     
